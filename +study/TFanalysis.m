classdef TFanalysis
    ...
    %
    % Works with study.run_tfr() outputs:
    ...
    %
    % Example:
    %   ds_tfd = study.Dataset(Out_tfd);                   % wrap TFR output
    %   tf = study.TFanalysis(ds_tfd) ...
    ...
    properties
        Dataset     % study.Dataset wrapping Out_tfd
    ...
    methods
        function obj = TFanalysis(dataset)
            if ~isa(dataset, 'study.Dataset')
                error('Input must be a study.Dataset (wrapping Out_tfd).');
            end
