...
% 
%   Out_tfd = study.run_tfr(dataset, Name, Value, ...)
% 
% High-level TFR runner for a study.Dataset with epoched data. Supports
...
%     Out_tfd_crop = study.run_tfr(ds, 'time_window',[-500 1500]);
% 
...
%     % end
%     Out_tfd_custom = study.run_tfr(ds, 'method',@my_cwt, 'params',struct('f',[2 80]));
% 
...
%
    % --- Input Parser ---
    p = inputParser;
    addRequired(p, 'dataset', @(x) isa(x, 'study.Dataset'));
