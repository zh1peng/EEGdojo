classdef ERPanalysis
    % ERPANALYSIS The main user-facing class for running an EEG analysis pipeline.
    % This class orchestrates the entire analysis, from data selection to
    % computation, plotting, and feature extraction. It holds the dataset,
    % user selections (groups, conditions, ROIs, etc.), and all results.

    properties
        Dataset     % A study.Dataset object containing the data and metadata.
    ...
    methods
        function obj = ERPanalysis(dataset)
            if ~isa(dataset, 'study.Dataset')
                error('Input must be a study.Dataset object.');
            end
            obj.Dataset = dataset;
                ...
                for s = 1:numel(subject_ids)
                    ...
                    for c = 1:numel(obj.Selection.Conditions)
                        ...
                        if ~isempty(trial_data)
                            ...
                            obj.Results.ERPs.(['sub_' subject_id]).(condition_name) = erp_data;
                        end
                    end
                end
            end
            ...
            for g = 1:num_groups
                ...
                for c = 1:numel(condition_names)
                    ...
                    if isfield(obj.Results.GA.(group_name).(condition_name), 'erp')
                        ...
                        topoplot(topo_data, obj.Dataset.chanlocs, 'electrodes', 'on');
        ...
        function plot_contrast_erp(obj, contrast_name, varargin)
            ...
            addParameter(p, 'sig_alpha', 1, @isnumeric);
    ...
    methods (Access = private)
        ...
        function plot_contrast_topos_for_windows(obj, contrast_name, time_windows)
            ...
            for i = 1:num_windows
                ...
                topoplot(topo_data_pos, obj.Dataset.chanlocs, 'electrodes', 'on');
                ...
                topoplot(topo_data_neg, obj.Dataset.chanlocs, 'electrodes', 'on');
                ...
                topoplot(topo_data_diff, obj.Dataset.chanlocs, 'electrodes', 'on');
