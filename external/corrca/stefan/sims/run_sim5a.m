...
for in = 1:length(Ns)
  ...
  for irep = 1:Nrep
    ...
    tic
    [K_shift(in, irep), p, W, ISC(:, in, irep), Y, A, gam, ISC_test(:, in, irep)] = ...
      est_K_shift(data, Nshuffle, gam, data_test);
    t1 = toc;

    tic
    K_phase(in, irep) = est_K_theiler(data, Nshuffle, gam);
    t2 = toc;
    
    tic
    K_param(in, irep) = est_K_parametric(data, Nsplits, gam);
    t3 = toc;
    
    K_spectral(in, irep) = sum(sign(ISC(:, in, irep)));

    A_acc(in, irep) = mean(max(abs(corr(A(:, 1:K), A_true'))));
    A_subspace(in, irep) = 1-subspace(A(:, 1:K), A_true')/(pi/2);
    A_rot = A(:, 1:K)*(A(:, 1:K)\A_true');
    A_subspace_regr(in, irep) = mean(diag(corr(A_rot, A_true')));
...
%     Y_true = reshape(permute(Y_true, [1 3 2]), T*N, K);
    ...
    Y_acc(in, irep) = mean(max(abs(corr(Y_true, Y_est))'));
    Y_subspace(in, irep) = 1-subspace(Y_est, Y_true)/(pi/2);
    Y_subspace_regr(in, irep) = mean(diag(corr(Y_rot, Y_true)));
  
    ISC_av(in, irep) = mean(ISC(1:K, in, irep));
    ISC_av_test(in, irep) = mean(ISC_test(1:K, in, irep));

    [in irep t1 t2 t3]
  end
end

save('mat/sim5a_results', 'K_shift', 'K_phase', 'K_param', 'K_spectral', 'A_acc', 'A_subspace', 'A_subspace_regr', ...
  'Y_subspace', 'Y_subspace_regr', 'Y_acc', 'ISC_av', 'ISC_av_test', ...
  'ISC', 'ISC_test', 'Nrep', 'Nshuffle', 'Nsplits', 'Ns', 'D', 'T', 'K', 'db_snr', 'snr', 'gam')
